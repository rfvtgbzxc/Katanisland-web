...
class Game_Test(WebsocketConsumer):
    ...
    def receive(self, text_data):
        ...
        if(evt["type"]=="mes_action"):
            ...
            if(evt["message"]["val"][0]==0 and evt["message"]["val"][1]==2):
                msg={"type":"mes_action","message":{"val":[0,3,random.randint(1,6),random.randint(1,6),random.randint(0,1)*random.randint(1,3)]}}
            ...
            if(evt["message"]["val"][0]==9 and evt["message"]["val"][1]==0):
                ...
                evt["message"]["val"].append(random.randint(1,6))
                msg=evt
            if("randoms" in msg["message"]):
                for i in range(0,len(msg["message"]["randoms"])):
                    msg["message"]["randoms"][i]=random.randint(0,msg["message"]["randoms"][i]-1)
        async_to_sync(self.channel_layer.group_send)(
